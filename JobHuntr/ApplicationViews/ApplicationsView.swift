...
struct ApplicationsView: View {
    ...
    @ObservedObject var applicationsManager: UserApplicationManager
    
    var body: some View {
        ZStack {
            AppColors.background.ignoresSafeArea()
            ScrollView {
                if applicationsManager.records.isEmpty {
                    ListCard {
                        Text("No Applications Found.")
                    }
                } else {
                    ForEach($applicationsManager.records, id: \.id) { $application in
    ...
    func deleteApplication(application: Application) {
        Task {
            await applicationsManager.delete(record: application)
        }
    }
...
struct ApplicationsView_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView {
            ApplicationsView(applicationsManager: UserApplicationManager())
                .environmentObject(UserManager())
        }
    }
}
